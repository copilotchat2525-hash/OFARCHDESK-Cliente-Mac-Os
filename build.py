...
def copy_sciter_macos_if_any(dst_frameworks_dir: Path) -> bool:
    """Copia libsciter.dylib si existe en ubicaciones típicas para macOS."""
    candidates = [
        Path("libsciter.dylib"),
        Path("libs/sciter/libsciter.dylib"),
        Path("res/libsciter.dylib"),
    ]
    for c in candidates:
        if c.exists():
            dst_frameworks_dir.mkdir(parents=True, exist_ok=True)
            shutil.copy2(c, dst_frameworks_dir / "libsciter.dylib")
            return True
    return False
...
def run_generate_and_make_installer(staging_dir: Path, app_exe_name: str, version: str) -> Path:
    ...
    if pa and final_installer.exists():
        system2(
            f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx '
            f'/t http://timestamp.digicert.com "{final_installer}"'
        )
...
def build_windows_non_flutter(version: str, features: str):
    ...
    pa = os.environ.get('P')
    if pa and exe_path.exists():
        system2(
            f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx '
            f'/t http://timestamp.digicert.com "{exe_path}"'
        )
    else:
        print('Not signed')
...
def build_macos_non_flutter(version: str, features: str):
    # 1) Compilar binario Rust para macOS (host debe ser macOS)
    cmd = 'cargo build --release'
    if features:
        cmd += f' --features {features}'
    system2(cmd)
    ...
    # 2) Estructura .app
    app_bundle = DIST_DIR / f"{APP_EXE}.app"         # p.ej. OFARCHDESK.app
    contents   = app_bundle / "Contents"
    macos_dir  = contents / "MacOS"
    frw_dir    = contents / "Frameworks"
    res_dir    = contents / "Resources"
    macos_dir.mkdir(parents=True, exist_ok=True)
    frw_dir.mkdir(parents=True, exist_ok=True)
    res_dir.mkdir(parents=True, exist_ok=True)

    # 3) Copiar ejecutable dentro del bundle
    app_bin = macos_dir / APP_EXE
    shutil.copy2(exe_path, app_bin)
    system2(f'chmod +x "{app_bin}"')

    # 4) Copiar libsciter.dylib
    ok_sciter = copy_sciter_macos_if_any(frw_dir)
    if not ok_sciter:
        print("[WARN] No se encontró libsciter.dylib local. Intentando descargar...")
        # Descarga directa desde el repo oficial si existe curl; si falla, pide colocarla manualmente
        try:
            system2(
                'curl -L -o "libsciter.dylib" '
                'https://github.com/c-smile/sciter-sdk/raw/master/bin.osx/libsciter.dylib'
            )
            shutil.copy2("libsciter.dylib", frw_dir / "libsciter.dylib")
        except SystemExit:
            raise SystemExit("[ERROR] No se pudo obtener libsciter.dylib. Colócala en res/ o libs/sciter/ y reintenta.")

    # 5) Ajustar rpath para que el binario encuentre Frameworks
    # En muchos casos Sciter ya usa @rpath, pero añadimos el rpath por seguridad
    system2(f'install_name_tool -add_rpath "@executable_path/../Frameworks" "{app_bin}"')
    # También podemos normalizar el id del dylib
    system2(f'install_name_tool -id "@rpath/libsciter.dylib" "{frw_dir / "libsciter.dylib"}"')
...
<plist version="1.0"><dict>
  <key>CFBundleName</key><string>OFARCHDESK</string>
  <key>CFBundleDisplayName</key><string>OFARCHDESK</string>
  <key>CFBundleIdentifier</key><string>com.ofarch.ofarchdesk</string>
...
""", encoding="utf-8")
    ...
    # 7) Firma ad-hoc opcional para facilitar ejecución local (no sustituye un Developer ID)
    # Si tienes certificado Developer ID, reemplaza "-" por tu identidad y añade --timestamp
    try:
        system2(f'codesign --force --deep -s - "{app_bundle}"')
    except SystemExit:
        print("[warn] codesign ad-hoc falló. Continuando sin firmar.")

    # 8) Crear DMG
    dmg_path = DIST_DIR / f"{APP_EXE}-{version}.dmg"
    volname = "OFARCHDESK"
    # hdiutil: empaqueta el .app en un DMG comprimido
    system2(f'hdiutil create -volname "{volname}" -srcfolder "{app_bundle}" -ov -format UDZO "{dmg_path}"')

    print(f"output location: {dmg_path}")
    return dmg_path
...
def main():
    ...
    if windows:
        ...
        # macOS sin Flutter
        build_macos_non_flutter(version, features)

    else:
        print("Plataforma no soportada en este build.py.")
